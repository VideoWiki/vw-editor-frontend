    ...
    <form v-if="currentTabIndex === 0">
      <vx-card :title="$t('studio.text.t1')" class="text-window">
        <div class="text-left">
          ...
          <span>
            <h6 v-if="displayMessage" class="text-danger pb-4">
              To write more than 1000 characters, Subscribe To Premium<br />
            </h6>
          </span>
        </div>
  ...
  methods: {
    ...
    textDisplay() {
      if (this.scriptError === false) {
        ...
        this.$vs.notify({
          title: 'ALERT:',
          text: 'To write a longer Script, Please Subscribe To Premium',
          color: 'warning',
        });
      }
    },
    setDescription() {
      ...
      this.$store.dispatch('studio/summarizeScript').then((res) => {
        console.log('description set', res.data.choices[0].text);
      });
    },
    createScenes() {
      this.$Progress.start();
